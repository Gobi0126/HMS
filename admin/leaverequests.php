...
(min-device-width: 768px) and (max-device-width: 1024px)  {
  ...
  <body>
    ...
    <?php
      ...
      function approve() {
        $conn= mysqli_connect('localhost','root','Gobi@1234','hms') or die("Connection failed:" .mysqli_connect_error());
        $regno1=$_POST['id'];
        $sql="SELECT * FROM leaverequests ";
      $query=mysqli_query($conn,$sql);     
      $a=1;
      ...
      {
         $f1=$rows['fromdate'];
     $sql1="UPDATE `leaverequests` SET status='approved' where regno='$regno1' and fromdate='$f1' and status='Pending'";
      ...
      function reject() {
       ...
       $sql1="UPDATE `leaverequests` SET status ='rejected' where regno= '$regno1' and fromdate='$f1' and status='Pending' ";   
        $query1=mysqli_query($conn,$sql1);
        if($query1){
          ?>
        <?php
